...
def main_menu_kb():
    ...
    b.button(text="🧪 اختبر نفسك",        callback_data="test_yourself")
    b.button(text="⚔️ تحدي صديق",         callback_data="challenge_menu")
    b.button(text="🏆 اختبار مشترك",      callback_data="group_menu")
    b.button(text="🔍 مراجعة أخطائي",     callback_data="my_mistakes")
...
def subjects_for_branch_kb(branch_key: str, prefix="subject"):
    ...
    # تحسين الرجوع بناءً على الـ prefix لضمان عمل زر الرجوع في الاختبار المشترك
    back_callback = "group_menu" if prefix == "group_create" else "subjects_menu"
...
@dp.callback_query(F.data == "test_yourself")
async def test_yourself(callback: CallbackQuery):
    await subjects_menu(callback)
...
async def handle_solo_answer(callback: CallbackQuery, state: FSMContext):
    ...
    if correct:
        score += 1
        db.remove_correct_answer(callback.from_user.id, q["q"])
    else:
        db.save_wrong_answer(
            callback.from_user.id,
            key if not key.startswith("branch_") else q.get("_subject", key),
            q["q"], q["options"][q["answer"]], q["options"][chosen]
        )
...
async def launch_group_quiz(chat_id: int, sid: str):
    ...
    # إنشاء رسالة الاختبار الأساسية التي سيتم تحديثها
    main_msg = await bot.send_message(chat_id, f"🚀 *جولة المثابر الوزاري بدأت!*\n{si} *{sn}* — {total} أسئلة\n⏱ 15 ثانية لكل سؤال\n\nاستعدوا... 3️⃣2️⃣1️⃣", parse_mode="Markdown")
    session["msg_id"] = main_msg.message_id
    await asyncio.sleep(3)
    ...
    for q_idx, q in enumerate(qs):
        ...
        # تحديث الرسالة الأساسية بالسؤال الجديد
        try:
            await bot.edit_message_text(chat_id=chat_id, message_id=main_msg.message_id,
                                        text=build_group_q(15), parse_mode="Markdown", reply_markup=answer_kb())
        except TelegramBadRequest:
            pass
        ...
        # تحديث نفس الرسالة بالنتيجة بدلاً من إرسال رسالة جديدة
        try:
            await bot.edit_message_text(chat_id=chat_id, message_id=main_msg.message_id,
                                        text=res, parse_mode="Markdown")
        except TelegramBadRequest:
            pass
        await asyncio.sleep(3)
    ...
    try:
        await bot.edit_message_text(chat_id=chat_id, message_id=main_msg.message_id,
                                    text=final, parse_mode="Markdown", reply_markup=b.as_markup())
    except TelegramBadRequest:
...
# ═══════════════════════════════════════════════
@dp.callback_query(F.data == "my_mistakes")
async def my_mistakes(callback: CallbackQuery):
    uid = callback.from_user.id
    count = db.get_wrong_answers_count(uid)
    if count == 0:
        await callback.message.edit_text(
            "🔍 *مراجعة أخطائي*\n━━━━━━━━━━━━━━━\n\n✅ ما عندك أخطاء مسجلة!\nاستمر بالمراجعة.",
            parse_mode="Markdown", reply_markup=back_home_kb()
        ...
        await callback.answer()
        return
    await show_mistake(callback.message, uid, 0, edit=True)
    await callback.answer()

async def show_mistake(message, uid, idx, edit=False):
    wrongs = db.get_wrong_answers(uid, limit=50)
    total = len(wrongs)
    if idx >= total:
        idx = total - 1
    w = wrongs[idx]
    from questions import SUBJECTS
    subj_name = SUBJECTS.get(w["subject"], {}).get("name", w["subject"])
    subj_icon = SUBJECTS.get(w["subject"], {}).get("icon", "📚")
    text = (
        f"🔍 *مراجعة أخطائي* — {idx+1}/{total}\n"
        f"━━━━━━━━━━━━━━━\n"
        f"{subj_icon} *{subj_name}*\n\n"
        f"❓ *{w['question']}*\n\n"
        f"❌ إجابتك: {w['chosen_ans']}\n"
        f"✅ الصحيحة: *{w['correct_ans']}*\n\n"
        f"📅 _{w['saved_at']}_"
    )
    b = InlineKeyboardBuilder()
    if idx > 0:
        b.button(text="→ السابق", callback_data=f"mistake:{idx-1}")
    if idx < total - 1:
        b.button(text="التالي ←", callback_data=f"mistake:{idx+1}")
    b.button(text="← رجوع", callback_data="back_home")
    b.adjust(2, 1)
    if edit:
        await message.edit_text(text, parse_mode="Markdown", reply_markup=b.as_markup())
    else:
        await message.answer(text, parse_mode="Markdown", reply_markup=b.as_markup())

@dp.callback_query(F.data.startswith("mistake:"))
async def navigate_mistake(callback: CallbackQuery):
    idx = int(callback.data.split(":")[1])
    await show_mistake(callback.message, callback.from_user.id, idx, edit=True)
    await callback.answer()
    await callback.answer()
