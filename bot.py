...
def main_menu_kb():
    ...
    b.button(text="💡 نصيحة اليوم",         callback_data="tip_of_day")
    # ── التحديات
...
async def auto_answer(callback: CallbackQuery):
    ...
    # تحقق إذا السؤال انجاوب من شخص قبله
    q_data = auto_quiz_answered.get(msg_id)
    if q_data and q_data["answered"]:
        await callback.answer("⚡ سبقك شخص آخر بالإجابة!", show_alert=True)
        return

    correct = chosen == correct_idx
    correct_label = f"{labels[correct_idx]}) {q_data['options'][correct_idx] if q_data else ''}"

    # قفل السؤال
    if q_data:
        auto_quiz_answered[msg_id]["answered"] = True

    # إعطاء XP إذا صحيح
    if correct:
        db.update_stats(user.id, 5, 1, 1, subj_key)
        await callback.answer("✅ صحيح! +5 XP 🎉", show_alert=False)
        result_line = f"✅ *{user.first_name}* أجاب صح! +5 XP 🎉"
    else:
        db.save_wrong_answer(user.id, subj_key,
                             q_data["question"] if q_data else "",
                             q_data["options"] if q_data else [],
                             correct_idx)
        await callback.answer(f"❌ خطأ! الصحيحة: {labels[correct_idx]}", show_alert=True)
        result_line = f"❌ *{user.first_name}* أجاب خطأ"

    # تحديث رسالة السؤال بالنتيجة
    opts_text = "\n".join(
    ...
    try:
        await callback.message.edit_text(
            f"🎯 *سؤال المثابر التلقائي*\n\n"
            f"❓ *{q_data['question'] if q_data else ''}*\n\n"
            f"{opts_text}\n\n"
            f"━━━━━━━━━━━━━━━\n"
            f"{result_line}\n"
            f"✅ الإجابة الصحيحة: *{correct_label}*",
