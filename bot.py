...
async def run_solo_q(chat_id, state: FSMContext):
    ...
    for t in range(14, -1, -1):
        ...
        if not data.get("timer_running"):
            return  # تم الإجابة — handle_solo_answer يتولى الأمر
...
async def handle_solo_answer(callback: CallbackQuery, state: FSMContext):
    ...
    # عرض النتيجة + الإجابة الصحيحة فوراً
    if correct:
...
async def settings_menu(callback: CallbackQuery):
    b = InlineKeyboardBuilder()
    b.button(text="📢 قناة المثابر",      url="https://t.me/AlMuthabir")
    b.button(text="💡 للاقتراح والتواصل", url="https://t.me/AlmuthabirBot")
    b.button(text="← رجوع",              callback_data="back_home")
    b.adjust(1)
    await callback.message.edit_text(
        "⚙️ *الإعدادات*\n"
        ...
        "_(قريباً)_\n\n"
        "📢 *قناة البوت:*\n"
        "[t.me/AlMuthabir](https://t.me/AlMuthabir)\n\n"
        "💡 *للاقتراح والتواصل:*\n"
        "@AlmuthabirBot",
        parse_mode="Markdown",
        reply_markup=b.as_markup()
