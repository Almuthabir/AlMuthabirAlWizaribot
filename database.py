...
def save_wrong_answer(user_id, subject, question, correct_ans, chosen_ans):
    conn = get_conn()
    today = date.today().isoformat()
    ...
    if existing:
        conn.execute("UPDATE wrong_answers SET saved_at=? WHERE id=?", (today, existing["id"]))
    else:
        conn.execute(
            "INSERT INTO wrong_answers (user_id, subject, question, correct_ans, chosen_ans, saved_at) VALUES (?, ?, ?, ?, ?, ?)",
            (user_id, subject, question, correct_ans, chosen_ans, today)
        )
    conn.commit(); conn.close()

def remove_correct_answer(user_id, question):
    conn = get_conn()
...
def get_wrong_answers(user_id, limit=50):
    conn = get_conn()
    rows = conn.execute(
        "SELECT * FROM wrong_answers WHERE user_id=? ORDER BY saved_at DESC LIMIT ?",
        (user_id, limit)
    ).fetchall()
    conn.close(); return [dict(r) for r in rows]

def get_wrong_answers_count(user_id):
    conn = get_conn()
    count = conn.execute(
        "SELECT COUNT(*) as c FROM wrong_answers WHERE user_id=?", (user_id,)
    ).fetchone()["c"]
