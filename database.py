...
def save_wrong_answer(user_id: int, subject: str, question: str, options: list, answer: int):
    import json
    conn = get_conn()
    today = date.today().isoformat()
    # تجنب التكرار
    existing = conn.execute(
        "SELECT id FROM wrong_answers WHERE user_id=? AND question=?",
        (user_id, question)
    ).fetchone()
    if not existing:
        conn.execute(
            "INSERT INTO wrong_answers (user_id, subject, question, options, answer, added_at) VALUES (?, ?, ?, ?, ?, ?)",
            (user_id, subject, question, json.dumps(options, ensure_ascii=False), answer, today)
        )
        conn.commit()
    conn.close()

def get_wrong_subjects(user_id: int) -> list:
    conn = get_conn()
    rows = conn.execute(
        "SELECT DISTINCT subject, COUNT(*) as cnt FROM wrong_answers WHERE user_id=? GROUP BY subject",
        (user_id,)
    ).fetchall()
    conn.close()
    return [dict(r) for r in rows]

def get_wrong_questions(user_id: int, subject: str) -> list:
    import json
    conn = get_conn()
    rows = conn.execute(
        "SELECT question, options, answer FROM wrong_answers WHERE user_id=? AND subject=? ORDER BY RANDOM() LIMIT 10",
        (user_id, subject)
    ).fetchall()
    conn.close()
    result = []
...
def delete_wrong_answer(user_id: int, question: str):
    conn = get_conn()
    conn.execute("DELETE FROM wrong_answers WHERE user_id=? AND question=?", (user_id, question))
    conn.commit()
    conn.close()
...
def get_streak_leaderboard(limit: int = 10) -> list:
    conn = get_conn()
    rows = conn.execute(
        "SELECT name, xp, streak FROM users ORDER BY streak DESC, xp DESC LIMIT ?", (limit,)
    ).fetchall()
    conn.close()
    return [dict(r) for r in rows]
